...
export default function AddTransaction() {
  ...
  const [amount, setAmount] = useReducer((state: string, newValue: string) => {
    ...
    firestore
      ...
      .update({
        main: firebase.firestore.FieldValue.increment(
          parseFloat(amount) * (transactionType === "deposit" ? 1 : -1)
        ),
      });
  ...
  return (
    <form className={styles.container} onSubmit={submit}>
      ...
      <div className={styles.radio}>
        <div className={`${styles.radioOverlay} ${styles[transactionType]}`}>
          <div>
            <span>Deposit</span>
          </div>
          <div>
